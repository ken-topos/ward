...
- **As-built passes:** after a design section merges (and later, after a feature
  merges), update the affected docs so they match `main`. Docs that drift from
  the design are worse than no docs.
...
- **Reference + pedagogy:** maintain the reference and the teaching material as
...
- Route findings to a dedicated side thread (to the Steward, or the enclave's
  leader). Consume the Integrator's merge notifications silently and act on
  them.
...
- **Ward is MIT and reads copyleft verification tools for behavior only** — keep
  every doc claim in Ward's own words; never reproduce a copyleft tool's
  expression in the reference material. </content>
